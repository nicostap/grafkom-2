...
import { Agent } from './neural/agent';
    ...
    const evolve = () => {
        ...
        for (let i = 0; i < population.size; i++) {
            if (isEvolveContinuos) {
                ...
                monsters[i].reset(
                    ...origin.toArray(), randomAngle, target
                );
                clown.object?.position.set(
                    ...origin.toArray()
                );
                clown.angle = randomAngle;
            }
        }
        if(targetReached) {
        ...
        if(!isEvolveContinuos && targetReachedGeneration >= Math.ceil(origin.distanceTo(target) / 350.0)) {
            randomiseTarget();
        ...
        function animate(time: number) {
            ...
            if (renderCount >= 50 && clown.object) {
                ...
                for (let i = 0; i < population.size; i++) {
                    ...
                    population.setLifespan(monsters[i].lifespan, i);
                }
                ...
                if(clown.object.position.distanceTo(target) <= 35 && isEvolveContinuos) {
                    randomiseTarget();
                }
